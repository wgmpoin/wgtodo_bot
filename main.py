...
OWNER_ID = int(os.getenv("OWNER_ID"))  # Telegram user ID kamu
...
async def start_add_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_user_registered(user_id):
        await update.message.reply_text("Maaf, kamu tidak terdaftar untuk menggunakan bot ini.")
...
async def input_judul(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👥 Siapa penerima tugas? (pisahkan dengan spasi, tanpa @)")
    return PENERIMA

async def input_penerima(update: Update, context: ContextTypes.DEFAULT_TYPE):
    task_data[update.effective_user.id]['penerima'] = update.message.text.strip()
    await update.message.reply_text("⏰ Deadline tugas? (format: YYYY-MM-DD HH:MM)")
    return DEADLINE
...
async def input_deadline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("Format salah. Coba lagi: YYYY-MM-DD HH:MM")
        return DEADLINE

async def input_keterangan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    task_data[user_id]['keterangan'] = update.message.text.strip()
    data = task_data[user_id]
